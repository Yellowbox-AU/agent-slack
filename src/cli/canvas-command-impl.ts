...
function printHelp(): void {
  ...
  agent-slack canvas create --title TITLE --from <file|-> [--channel C]
...
async function commandCreate(parsed: ParsedArgs): Promise<void> {
  ...
  const privateRequest = await performPrivateEdit(workspace, model, (latest) =>
    buildPrivateMarkdownDocumentData({
      ...privateDocumentInput(latest, markdown, "replace"),
      title,
    }),
  );
  const after = await fetchCanvasModelOrFallback(workspace, canvasId, title);
  const verified =
...
async function commandTitle(parsed: ParsedArgs): Promise<void> {
  ...
  const privateRequest = await performPrivateEdit(workspace, before, (latest) =>
    buildPrivateMarkdownDocumentData({ ...privateDocumentInput(latest, "", "replace"), title }),
  );
  ...
  writeOutput({
    ...
    title,
    idempotency_key: key,
    result: { status: "updated", verified: displayTitle(after) === title },
    private_request: privateRequest,
  });
}
