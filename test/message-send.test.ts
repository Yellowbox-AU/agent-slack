...
describe("editMessage: inline entities and lists", () => {
  test("edits an inline mailto link as a rich-text link", async () => {
    const calls: { method: string; params: Record<string, unknown> }[] = [];
    const ctx = createContext(calls);
  ...
  test("edits an inline usergroup mention as a rich-text usergroup element", async () => {
    const calls: { method: string; params: Record<string, unknown> }[] = [];
    const ctx = createContext(calls);
